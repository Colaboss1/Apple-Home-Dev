...
export class SectionReorderManager {
  ...
  private async prepareSectionsData(areas: any[], hass: any): Promise<SectionItem[]> {
    ...
    let hasDef = false; try { const e = hass.entities ? Object.values(hass.entities) : [], d = hass.devices ? Object.values(hass.devices) : []; hasDef = e.some((x: any) => !x.area_id && (!x.device_id || !d.find((y: any) => y.id === x.device_id)?.area_id)); } catch { hasDef = true; }
